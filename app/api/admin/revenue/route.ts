...
export async function GET(request: NextRequest) {
  try {
    ...
    const totalBookings = revenue.reduce(
      (sum: any, r: { totalBookings: any }) => sum + r.totalBookings,
      0,
    );
    const totalClients = revenue.reduce(
      (sum: any, r: { totalClients: any }) => sum + r.totalClients,
      0,
    );
    ...
    return apiError("Revenue GET Error", error, "Internal server error", 500);
  }
}
